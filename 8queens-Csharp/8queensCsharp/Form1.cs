...
namespace WindowsFormsApplication1
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void btnTreeAlgorithm_Click(object sender, EventArgs e)
        {
            ...
            // Kodi qe deshironi per ta matur vjen ketu
            // Kodi qe deshironi per ta matur vjen ketu
            listZgjidhjet.Items.Clear();
            BackTracking.NumriTentimeve = 0;
            BackTracking.NumriZgjidhjeve = 0;
            BackTracking.Zgjidhjet.Clear();
            Stopwatch sw = Stopwatch.StartNew();
            new BackTracking(0, -1, null).EcniNeperPeme();
            sw.Stop();
            for(int i = 0; i < BackTracking.Zgjidhjet.Count; i++)
            {

                listZgjidhjet.Items.Add(BackTracking.Zgjidhjet[i].ToUpper());
            }
            label5.Text= BackTracking.NumriTentimeve.ToString("#,##0");
            label6.Text = BackTracking.Zgjidhjet.Count.ToString("#,##0");
        ...
        {
            listZgjidhjet.Items.Clear();
            BranchAndBound.NumriTentimeve = 0;
            BranchAndBound.NumriZgjidhjeve = 0;
            BranchAndBound.Zgjidhjet.Clear();
            Stopwatch sw = Stopwatch.StartNew();
            new BranchAndBound().zgjidh8Mbretereshat();
            sw.Stop();
            for (int i = 0; i < BranchAndBound.Zgjidhjet.Count; i++)
            {

                listZgjidhjet.Items.Add(BranchAndBound.Zgjidhjet[i].ToUpper());
            }
            label5.Text = BranchAndBound.NumriTentimeve.ToString("#,##0");
            label6.Text =BranchAndBound.Zgjidhjet.Count.ToString("#,##0");
            label8.Text = sw.ElapsedMilliseconds.ToString();
